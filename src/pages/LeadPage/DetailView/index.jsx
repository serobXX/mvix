...
const DetailView = ({
  ...
  const { data, isFetching } = useGetLeadByIdQuery(id, {
  ...
  const cards = useMemo(
    () => ({
      ...
      entity_activity: (
        <EntityActivityCard
          id={id}
          entity={entityValues.lead}
          parentUrl={removeAbsolutePath(routes.leads.toView(id, view))}
          item={item}
        />
      ),
      attachments: (
      ...
      <Routes>
        {activityPermission.update && (
          <Route
            ...
            element={
              <AddEditActivity
                closeLink={routes.leads.toView(id, view)}
                fromDetailView
                hideRelatedFields
              />
            }
